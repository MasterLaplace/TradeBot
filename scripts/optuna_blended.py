...
def walk_forward_mean_blend(blend, sub_params, n_splits=3, transaction_fees=0.0001, slippage=0.0, fixed_cost_per_trade=0.0):
...
def objective(trial: optuna.Trial, transaction_fees: float = 0.0001, slippage: float = 0.0, fixed_cost_per_trade: float = 0.0):
    ...
    score = walk_forward_mean_blend(blend, sub_params, n_splits=3, transaction_fees=transaction_fees, slippage=slippage, fixed_cost_per_trade=fixed_cost_per_trade)
    return score
...
if __name__ == '__main__':
    ...
    # Wrap the objective to carry transaction fees
    ...
    with open('experiments/optuna/blended_optuna_best.json', 'w') as f:
