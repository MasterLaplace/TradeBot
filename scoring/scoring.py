...
def backtest(
    ...
    for i in nb_units.index:
        ...
        if i == 0:
            ...
            traded_notional = np.abs((ideal_nb_units - prev_nb_units) * current_prices).sum()
            transaction_costs = traded_notional * transaction_fees
            # Slippage as proportion of traded notional
            slippage_costs = traded_notional * slippage
            # Fixed fee per trade (number of assets that changed position)
            n_trades = int((np.abs(ideal_nb_units - prev_nb_units) > 0).sum())
            fixed_trade_costs = n_trades * fixed_cost_per_trade
            total_tc = transaction_costs + slippage_costs + fixed_trade_costs
            capital_after_tc = capital_before_rebalance - total_tc
...
def show_result(local_score: dict, is_show_graph: bool = False, save_path: str | None = None):
    ...
    if is_show_graph or save_path:
        ...
        plt.legend(loc='best')
        plt.tight_layout()
        # If save_path provided, save instead (or in addition) to show
        if save_path:
        ...
        if is_show_graph:
            try:
                ...
                print("⚠️ Impossible d'afficher le graphique en mode interactif sur cet environnement. Essayez --save-graph pour sauvegarder l'image ou exécuter sur une machine avec X11 / GUI.")
